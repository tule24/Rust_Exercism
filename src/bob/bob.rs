pub fn bob() {}
pub fn reply_2(message: &str) -> &str {
    let message = message.trim();
    if message.is_empty() {
        return "Fine. Be that way!";
    }
    if !message.chars().any(|character| character.is_lowercase())
        && message.chars().any(|character| character.is_alphabetic())
    {
        if message.chars().last().unwrap().eq_ignore_ascii_case(&'?') {
...
pub fn reply(message: &str) -> &str {
    let is_yelling = |m:&str| {
        m.to_uppercase() == m
     && m.chars().any(|character| character.is_alphabetic())
    };
    match message.trim() {
