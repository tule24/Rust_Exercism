...
//         let mut real_minutes = minutes % 60;
//         let mut real_hours = (hours + minutes / 60) % 24;
...
fn all_signs<T: Copy + Neg<Output = T>>(a: T, b: T) -> [(T, T); 4] {
    [
        (a, b),
        (a, -b),
        (-a, b),
...
macro_rules! demo_euclid {
    ($a:expr, $b:expr) => {
        for (a, b) in all_signs($a, $b).iter() {
            let a = *a;
            let b = *b;

            println!("{:2} /          {:2}  = {:4}", a, b, a / b);
            println!("{:2}.div_euclid({:2}) = {:4}", a, b, a.div_euclid(b));
            println!("{:2} %          {:2}  = {:4}", a, b, a % b);
            println!("{:2}.rem_euclid({:2}) = {:4}", a, b, a.rem_euclid(b));
            println!();
        }
    };
}
...
impl Clock {
    pub fn new(hours: i64, minutes: i64) -> Clock {
        Clock {
            ...
            // ((X % Y) + Y) % Y ==  X.rem_euclid(Y)
            // Cho X: số bị chia
            // Y: số chia
            // R: kết quả
            // Z: số dư
            // => X = R * Y + Z với 0 <= Z < abs(Y)
        }
    }
