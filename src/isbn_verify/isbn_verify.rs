...
pub fn is_valid_isbn(isbn: &str) -> bool {
    let temp = isbn.char_indices().filter(|(_, c)| *c != ' ' || *c != '-').collect::<Vec<(usize, char)>>();
    if temp.len() != 10 {return false}
    let mut sum = 0;
    for (i, c) in temp{
         match (i, c) {
            (9, 'X') => sum += 10,
            (_, c) if !c.is_digit(10) => return false,
            (i, c) => sum += (10 - i as u32) * (c.to_digit(10).unwrap())
        }
    }
    sum%11 == 0
}
